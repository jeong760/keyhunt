...
int main(int argc, char** argv) {
	...
	char* filename = NULL, * precalculated_mp_filename = NULL, * hextemp, * aux, * aux2, * pointx_str, * pointy_str, * str_seconds, * str_total, * str_pretotal;
	FILE* fd;
	uint64_t j, total_precalculated, i, PERTHREAD, BASE, PERTHREAD_R, itemsbloom, itemsbloom2;
	int readed, continue_flag, check_flag, r, lenaux, lendiff, c;
		...
		*/
		...
		if (FLAGPRECALCUTED_P_FILE) {
...
#ifdef WIN64
				tid[i] = CreateThread(NULL, 0, thread_bPloadFile, (void*)&temp[i], 0, &s);
#else
			...
			for (i = 0; i < NTHREADS; i++) {
				...
				if (FLAGDEBUG) printf("[I] %lu to %lu\n", temp[i].from, temp[i].to);
#ifdef WIN64
				tid[i] = CreateThread(NULL, 0, thread_bPload, (void*)&temp[i], 0, &s);
#else
